...
Inspect the file by using the `View()` function. You will notice is that the column names are rather long. Second thing you will notice is that we probably do not need to keep all the columns. Let's drop the this column, and any other columns we do not need, and then rename the remaining columns for easier reference.
