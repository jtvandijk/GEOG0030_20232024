...
3. Click on **Wards (December 2022) Boundaries GB BGC**.
...
Inspect the file by using the `View()` function. You will notice is that the column names are rather long, so let's rename the columns for easier reference.
...
#| filename: "R code"
# rename columns
names(ethnicity_london) <- c('ward22cd','ward22nm','ladcd','ladnm','all_pop','white_british','white_irish',
                             'white_gypsy','white_roma','white_other','mixed_white_asian', 'mixed_white_african',
...
city_of_london_sdf <- city_of_london_sdf |>
  mutate_at(c(15:34), function(x) as.integer(x/nrow(city_of_london_sdf)))
...
ward_spatial_weights_rook <- ward_neighbours_rook |> nb2listw(style = 'W' )
...
::: {.callout-warning}
If we want to only map the statistically significant clusters, we need to use these [z-scores](https://en.wikipedia.org/wiki/Standard_score) to filter out only statistically significant clusters. When using a 95 percent confidence level your values should fall outside `-1.96` and `+1.96` standard deviations.
:::
