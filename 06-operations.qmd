...
Of course it should be of no surprise that our `msoa_lon` spatial dataframe is in **BNG / ESPG: 27700**, however, it is always good to check. Let's go ahead and read in our `tfl_stations` dataset as well:
...
You should see that our `london_stations` spatial dataframe is now in **BNG / EPSG: 27700**. We are now ready to load our final dataset - our `csv` that contains our crimedata for 2021. We can do this by repeating the steps we took during [Week 4's computer tutorial](04-statistics.html#crime-data):
