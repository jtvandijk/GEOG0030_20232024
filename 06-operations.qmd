...
Calculate the number of bike thefts per MSOA and the number of bike thefts within 400m of a station per MSOA using the PIP operation via `st_intersects()`:
...
As you can see from the code above, we have now calculated our total bike theft and bike theft near a train station for each MSOA. The final step in our processing therefore is to create our rate of potentially station-related bike theft = bike theft near train station / total bike theft.
