...
::: {.callout-warning}
There may be some small differences between the numbers in your output and the numbers shown here. If so, this is due to retrospective updates made to the police data after the data was downloaded for the generation of this year's Geocomputation workbook. These updates could include corrections to the crime's attribute information or LSOA assignment.
:::

::: {.callout-warning}
Whereas the number of crimes is very similar in the above example, there is still a difference of **10** crimes. This difference is caused by our last `filter()`, where 2011 LSOAs that have not been assigned a 2021 LSOA code are filtered out. In these cases there is actually a recording error and no valid 2011 LSOA code is used. We therefore choose to ignore these data points. 
:::
