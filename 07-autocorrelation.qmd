...
This week, we are using a completely new dataset and investigating a different phenomena: **clustering of different population groups**. We will be investigating to what extent people in London who [self-identified](https://www.ons.gov.uk/peoplepopulationandcommunity/culturalidentity/ethnicity/bulletins/ethnicgroupenglandandwales/census2021) as **Asian - Bangladeshi** in the 2021 Census are clustered in London at the Ward-level. 
...
To complete this analysis, we will be using a single data download from the [London Datastore](https://data.london.gov.uk/), which we will need to clean and join to a spatial layer containing the relevant Ward boundaries.

### Spatial analysis set up {#setup-w07}
Open a new script within your GEOG0030 project and save this script as `wk6-segregation-analysis.r`. At the top of your script, add the following metadata:
...
#| tidy: False
#| filename: "R code"
# Analysing segregation in London 
# Date: January 2024
# Author: Justin 
```
...
Open de file in Excel, and look at the first tab (**Front Page**). You will notice that under **Geography** 2022 Wards are listed. This is good because these are the ones we just downloaded. The actual data that we want to use can be found on the **2021** tab, which contains the 2021 Census results.

```{r}
#| label: fig-ethnic-excel
#| echo: False 
#| fig-cap: "The Excel file containing the number of people that self-identify as a particular group by Ward. [[Enlarge image]](https://jtvandijk.github.io/GEOG0030/images/w07/ethnic-groups.png){target='_blank'}"
knitr::include_graphics('images/w07/ethnic-groups.png')
```

We clearly need to extract the data and save this as a separate `csv` file before we can import the data into R.
...
After this, let's load our London MSOA file:
...
Inspect the file by using the `View()` function. First thing you will notice is that the column names are rather long. Second thing you will notice is that one of the columns does not contain any information but `NA` values. It also does not have a meaningful name (`...9`), and it seems that in the process of saving the `csv` an extra column was added by Excel in the conversion. Let's drop this column, and any other columns we do not need, first and then rename the remaining columns.
...
We have effectively taken out the **25** 2022 Wards that fall with in the City of London Local Authority District. We now need to assign our ethnic group data to this. We will add the data and equally divide the counts across the Wards.
...
To join the `ethnicity_london` dataset to the City of London wards is a bit tricky --- because the dataset does not contain the actual Wards codes. Fortunately for us, the London Data Store was a bit cheeky and in the file they created they put the Local Authority District code of the City of London into the Ward column. We can use this to join our datasets together with `left_join`.

```{r tidy='styler'}
#| label: 07-join-city-shp
#| classes: styled-output
#| echo: True
#| eval: True
#| filename: "R code"
...
```{r}
#| label: 07-city-of-london-divvie-up
#| classes: styled-output
#| echo: True
#| eval: True
#| tidy: True
#| filename: "R code"
# divide counts by Wards
city_of_london_sdf <- city_of_london_sdf |>
...
Today, we are interested in looking at spatial autocorrelation: the effect of spatial processes on distributions. We will be using our newly created `ethnicity_london_sdf` to look at this in action.
...
We looked at how we can use R to extract some of these summary statistics briefly in [Week 4's computer tutorial](04-statistics.html#atomic-vectors), but let's have a look at how we can add further to this EDA, including creating some statistical charts of our data's distribution. A simple and straightforward way is by using the `summary()` function.
...
We can actually see our data has a strong negative skew, with a some Wards having a high number of self-identified Asian-Bangladeshi's residing in them. This might be a first hint at some form of clustering taking place.
...
#| label: fig-boxplot
#| echo: False 
#| fig-cap: "Simple boxplot. [[Enlarge image]](https://jtvandijk.github.io/GEOG0030/images/w07/boxplot.png){target='_blank'}"
...
```{r tidy='styler'}
#| label: fig-07-facet-map-asian-bangladeshi
...
Three of the most frequently used tests are the Global Moran's I, the. Local Moran's I, and the Local Getis-Ord:
...
#| echo: False 
#| fig-cap: "A spatial lag model - plotting value against the mean of its neighbours. Source: [Manuel Gimond](https://mgimond.github.io/Spatial/spatial-autocorrelation.html). [[Enlarge image]](https://jtvandijk.github.io/GEOG0030/images/w07/moran-scatter.png){target='_blank'}"
knitr::include_graphics('images/w07/moran-scatter.png')
```
...
#| echo: False 
#| fig-cap: "Determining significance using a Monte Carlo simulation. Source: [Manuel Gimond](https://mgimond.github.io/Spatial/spatial-autocorrelation.html). [[Enlarge image]](https://jtvandijk.github.io/GEOG0030/images/w07/mc-sim.png){target='_blank'}"
knitr::include_graphics('images/w07/mc-sim.png')
```
...
- **Contiguity** neighbours: Queen / Rook
...
#| echo: False 
#| fig-cap: "Different approaches of conceptualising neighbours for spatial autocorrelation measurement: contiguity, fixed distance and nearest neighbours. Source: [Manuel Gimond](https://mgimond.github.io/Spatial/spatial-autocorrelation.html). [[Enlarge image]](https://jtvandijk.github.io/GEOG0030/images/w07/iffn.png){target='_blank'}"
...
```{r}
#| label: 07-who-is-your-neighbour
#| classes: styled-output
#| echo: True
#| eval: True
#| tidy: True
...
```{r}
#| label: fig-07-plot-them-neighbours-queen
...
```{r}
#| label: fig-07-plot-them-neighbours-rook
...
```{r}
#| label: fig-07-plot-them-neighbours-fixed
...
When comparing these different maps, we can see that there is definitely a difference in the number of neighbours when we use our different approaches. It seems our fixed distance neighbour conceptualisation has many connections in the centre of London versus areas on the outskirts. We can see that our contiguity approaches provide a more equally distributed connection map, with our Queen conceptualisation having a few more links that our Rook.
...
```{r}
#| label: 07-meeting-the-neigbhours-queen
#| classes: styled-output
#| echo: True
#| eval: True
#| tidy: True
...
ward_spatial_weights_queen <- ward_neighbours_rook |> nb2listw(style = 'C' )

# create a neighbours list
ward_spatial_weights_fd <- ward_neighbours_fd |> nb2listw(style = 'C', zero.policy = TRUE)
```
...
```{r}
#| label: fig-07-plot-moran-queens
...
The Moran’s I statistic calculated should be 0.73. With $1$ = clustered, $0$ = no pattern, $-1$ = dispersed, this means we can confirm that the population in London that self-identifies as Asian-Bangladeshi is positively autocorrelated. In other words, Asian-Bangladeshi's do spatially cluster. We can also consider the pseudo $p$-value as a measure of the statistical significance of the model - at $< 2.2e-16$, which confirm our result is statistically significant.
...
As you should see, we ae not given a single statistic as we did with our global test, but rather a table of different statistics that are all related back to each of the Wards in our dataset.
...
tm_shape(ethnicity_london_sdf) +
  ...
  tm_layout(main.title= "Spatial Autocorrelation Asian Bangladeshi's in London",
...
```{r tidy='styler'}
#| label: fig-07-map-local-moran-significance
...
# map our local # map the local Moran's I statistic
tm_shape(ethnicity_london_sdf) +
...
```{r tidy='styler'}
#| label: fig-07-map-local-moran-cluster-type
...
#| filename: "R code"
# significance breaks
ethnicity_london_sdf <- ethnicity_london_sdf |> mutate(obs_diff = (asian_bangladeshi_prop - mean(ethnicity_london_sdf$asian_bangladeshi_prop)))

# compare local LMI value with mean LMI value
ethnicity_london_sdf <- ethnicity_london_sdf |> mutate(LMI_diff = (ethnicity_london_sdf$LMI_Ii - mean(ethnicity_london_sdf$LMI_Ii)))
...
```{r}
#| label: 07-local-moran-cluster-types
#| classes: styled-output
#| echo: True
#| eval: True
#| tidy: True
...
```{r}
#| label: 07-local-moran-cluster-types-colours
#| classes: styled-output
#| echo: True
#| eval: True
#| tidy: True
...
tm_shape(ethnicity_london_sdf) +
  ...
  tm_layout(main.title = "Cluster Map of Asian Bangladeshi's in London",
...
# plot the different clusters
tm_shape(ethnicity_london_sdf) +
  tm_polygons("asian_bangladeshi_prop", style = "pretty", midpoint=0, title = "Local Gi* statistic") +
  tm_layout(main.title = "Hot/Cold Spot Map of Asian-Bangladeshi's",
            main.title.fontface = 2, fontfamily = "Helvetica",
            legend.outside = TRUE,
            legend.outside.position = "right",
            legend.title.size = 1,
...
Our map shows quite clear hot spots of self-identified Asian-Bangladeshi's across London. 
...
Through conducting our spatial autocorrelation tests, we can visually confirm the presence of clusters in our `asian_bangladeshi_prop` variable and provide a significance value associated with these tests. 

Now you have the code, you will be able to repeat this analysis on any variable in the future. For this week's assignment, we therefore want you to find out whether also our `white_british` variable contains spatial clustering.
...
3. Create a local Moran's I map showing the cluster types.
4. Create a local Getis-Ord hotspot map.
5. Compare these results to the output of our `asian_bangladeshi_prop` values. Do both variables have clusters? Do we see similar clusters in similar locations?
