...
#### Mapping crime data
Now you have both datasets ready, it is time to style the maps. Remember to use the **Properties** box to symbolise your mapss. Think through using an appropriate colour scheme - perhaps [have a look online for some examples](https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3) if you do not want to use the default settings. Once you are happy with their symbolisation, we can turn them into proper publishable maps using QGIS's **Print Layout**. 

::: { .callout-tip}
If you have used ArcMap before, this is similar to switch the view of your map canvas to a print layout within the main window but in QGIS's case, it loads up a new window.
::: 
