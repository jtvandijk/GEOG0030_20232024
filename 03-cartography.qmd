...
11. We now have our Borough crime rate dataset ready for mapping. We just now need to repeat this process to have our LSOA dataset. So, repeat the above processes to create a `crime-rate` and `crime-rate-int` column within the LSOA dataset as well.
