...
Looking at this we need to take two steps. First, we need to remove the top rows that we do not need. Second, we need to split the information in the first column into two columns. Currently, this columns contains unique area codes as well as unique area names. However, to be able to effectively link our population data to our spatial data, the codes are much more reliable and much less likely to cause issues than the area names. 
...
3. Cut all the data from the `pop2011` column that is stored in column **B** and past these into column **C**. You now should have a column without any data between the `lsoa` and `pop2011` columns.
...
#| echo: False 
#| cache: True
#| fig-cap: "Highlight the `lsoa` columns and find the **Text to Columns** menu option."
...
7. One thing that might cause problems when we try to link the population data to the spatial data is that there are still spaces contained in the `lsoa11_code` column. This is not directly noticeable until you select an individual cell, but they are definitely there. To solve this we can highlight the entire `lsoa11_code` column and in the **Home** menu opt for **Replace** under the **Find & Select** menu.
...
8. In the **Find what** box put in a singular `space`, keep the **Replace with** box empty, and click on **Replace all**. You should get a message that 4,642 replacements have been made. 
...
::: { .callout-tip}
After saving the file, Excel might give you a warning along the lines of *Possible data loss*. You can safely ignore this message as the only 'information' you have lost is markup information or formulas native to Excel. In some cases you might also have options to choose in terms of what type of `csv` you would want to save the spreadsheet as. If so, opt for something along the lines of `CSV UTF-8 (Comma-delimited) (.csv)`.
:::
