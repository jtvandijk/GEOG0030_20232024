...
This first computer practical is mainly concerned with creating **Attribute Joins** in order to create [choropleth maps](https://en.wikipedia.org/wiki/Choropleth_map). You will be using different types of *joins* throughout this practical, this module in general, and probably the rest of your career, so it is incredibly important that you understand how these work.
...
Today we will look at population in London in the Census years of 2011 and 2021 at the so-called **Lower Super Output Area** (LSOA) geography level (see @#02-download-spatial below for some details on Administrative Geographies).
...
### Cleaning population data {#w02-cleaning}
...
6. Cut all the data from the `pop2011` column that is stored in column **B** and paste these into column **C**. You now should have a column *without* any data sat between the `lsoa` and `pop2011` columns.
...
12. In the **Find what** box put in a singular `white space`, using the *spacebar* on your keyboard, keep the **Replace with** box empty, and click on **Replace all**. You should get a message that **4,835** replacements have been made. 
...
13. To format the `pop2011` column, highlight the entire column and *right-click* on the `C` cell.  Click on **Format Cells** and set the cells to **Number** with **0** decimal places. You should see that the commas are now removed from your population values.
...
- Export your joined dataset (London only) as a `GeoPackage` and save this into your `raw/boundaries` folder as `LSOA2021_London.gpkg`. <u>We will use this file in later weeks.</u>
